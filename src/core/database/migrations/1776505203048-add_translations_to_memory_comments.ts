...
export class AddTranslationsToMemoryComments1776505203048 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const [colRow] = (await queryRunner.query(
      `SELECT COUNT(*) AS cnt FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'memory_comments' AND COLUMN_NAME = 'translations'`,
    )) as [{ cnt: string }]
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`ALTER TABLE memory_comments DROP COLUMN IF EXISTS translations`)
  }
}
