...
- **QR Code**: Admin displays the QR code from the **Home page QR widget** (visible in the right panel on the Home page — click the widget to load today's QR code) on a screen. Employees scan it with their phone — they are redirected to the Clock page where attendance is recorded. The system validates the date, QR token, and client IP against the company's allowed IP list.
...
- **Home Page Clock Widget**: Employees can clock in/out directly from the Home page using the manual **Clock In / Clock Out** buttons when any of the following apply for today: **approved or pending WFH request**, **approved Business Trip**, **Permanent Remote** status, or **approved Overtime**. On all other days the widget shows "Scan QR" and "Face Check-In" buttons instead.
- **Pending WFH allowed**: An employee with a still-pending WFH request can use the clock widget — this prevents missed clock-ins when the approver hasn't reviewed the request yet.
