...
- **KYC Management**: Review face registration submissions from employees. Each card shows the employee's submitted face photo, name, and submission date. Use the **filter tabs** (All / Pending / Approved / Rejected) to organise the queue. Click **Approve** to activate face check-in for the employee, or **Reject** (with an optional reason) to send it back. You will receive a push notification when a new KYC is submitted. You can also cancel any employee's KYC from the **User Management** page (button appears when the employee has any KYC status) — this resets them to unregistered so they can re-submit.
