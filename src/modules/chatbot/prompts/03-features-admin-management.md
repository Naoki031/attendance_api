...
- **Attendance Logs**: View detailed daily attendance logs for all employees. Use the **month/year selectors**, **company filter**, and **user search** to narrow the list. The table shows each entry's user, date, schedule type (company default or custom), clock-in time, clock-out time, working duration, and Present/Absent status. Click **Export to Google Sheets** to push the filtered data (choose month, year, and company in the export dialog). Admins can **edit clock-in/clock-out times** for any entry (a reason is required). Every edit is saved to an **edit history**, reviewable per entry by clicking the history icon. For face check-in entries, click the **camera icon** to view the captured check-in photo along with the confidence score.
...
**Logs & Monitoring:**
- **Error Logs**: View system-level application errors. Stats cards show total, unresolved, and error-level counts at a glance. Use the **level filter** (info / warn / error / fatal) and **resolved filter** (resolved / unresolved) to narrow the list. You can **resolve** multiple entries at once using the batch resolve button (appears when rows are selected), or **purge old logs** to clean up stale records. Each entry includes the error message, level, HTTP status code, timestamp, and stack trace details.
...
**Configuration:**
- **Email Templates**: Manage email notification templates per company and per event type. Filter by company or template key. Create, edit, or delete templates — each template has a **key** (selects the event type), **company scope** (global — applies to all companies — or scoped to a specific company), **subject**, and **HTML body** (composed via a rich text editor). When editing, **available variable chips** for the selected key are shown — click a chip to insert the variable (e.g. `{{employee_name}}`) directly into the body. The template list shows a rendered HTML preview of each body.
